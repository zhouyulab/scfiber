...
compute_modularity <- function(gid, filter_cnt_mat, expr_cutoff, f_SNN){
  library(scran)
  load(f_SNN)
...
clnum <- detectCores()
cl <- makeCluster(getOption("cl.cores", min(ncore, clnum)))
print("Compute modularity")
res <- parLapply(cl, rownames(filter_cnt_mat), compute_modularity, filter_cnt_mat=filter_cnt_mat, expr_cutoff=expr_cutoff, f_SNN=f_SNN)
stopCluster(cl)
