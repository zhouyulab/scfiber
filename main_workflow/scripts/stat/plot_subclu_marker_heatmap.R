...
filter_marker_df <- function(block){
  WT_mat <- as.matrix(block[,30:39])
  FL_mat <- as.matrix(block[,40:49])
  head(WT_mat)
  head(FL_mat)
  ...
  block <- block[block$Cluster %in% c("C4.1", "C4.2", "C4.3", "C4.4", "C4.5"),]
  return(block[1:10,])
}
...
WT_mat <- as.matrix(filtered_marker_df[,30:39])
scaled_WT_mat <- WT_mat / apply(WT_mat, 1, function(x) max(x, na.rm = TRUE))

FL_mat <- as.matrix(filtered_marker_df[,40:49])
