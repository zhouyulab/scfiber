...
if (length(args) == 10) {
  ...
  f_corSCE <- args[3]
  f_SNN <- args[4]
  min_expr_cutoff <- as.numeric(args[5])
  k_nearest  <- as.integer(args[6])
  inner_ratio_cutoff <- as.numeric(args[7])
  tree_cutoff <- as.numeric(args[8])
  min_group_gene <- as.integer(args[9])
  f_out <- args[10]
} else {
  q()
}

# f_in <- "/home/sqreb/ias/data/CottonSingleCell/analysis/recluster/modularity/Modularity.filter.tsv"
# min_expr_cutoff <- 2
# tree_cutoff <- 0.5
# k_nearest <- 1
# inner_ratio_cutoff <- 0.3
# min_group_gene <- 5
...
print(names(mergeSCE)[1:3])
print(names(mergeSCE)[1:3])
print(marker_df$Gid[1:3])
print(sum(names(mergeSCE) %in% marker_df$Gid))
mergeSCE <- mergeSCE[names(mergeSCE) %in% marker_df$Gid,]
gc()
...
corrected_mat <- corSCE@assays@data$corrected

group_expr_mat <- expr_mat
for(i in 1:nrow(expr_mat)){
  similar_cell <- find_similar_cell(which(expr_mat[i,]), SNN_graph, k_nearest, inner_ratio_cutoff)
  if(length(similar_cell)==0) next()
  group_expr_mat[i, similar_cell] <- TRUE
}

gene_dist_mat <- dist(group_expr_mat, method = "manhattan")
gene_clu <- hclust(gene_dist_mat)
plot(gene_clu)
...
plot_UMAP <- function(cell_name_li, umapSCE, dim1, dim2, label){
  df <- data.frame(x=umapSCE@int_colData$reducedDims$UMAP[,dim1], y=umapSCE@int_colData$reducedDims$UMAP[,dim2], highlight=colnames(umapSCE)%in%cell_name_li)
  df$highlight <- factor(df$highlight, levels = c(TRUE, FALSE), labels = c("In", "Out"))
  col_li <- c("red", "grey70")
  names(col_li) <- c("In", "Out")
...
for(g in gene_group_info$Label){
  ...
  group_expr_cell_li <- names(which(colSums(expr_mat[rownames(expr_mat) %in% gid_li,]) > 0 ))
  for(indx in 1:3){
    dimx <- dimx_li[indx]
    dimy <- dimy_li[indx]
    ps[[indx]] <- plot_UMAP(group_expr_cell_li, corSCE, dimx, dimy, g)
  }
  pdf(file.path(plot_dir, sprintf("%s.UMAP.pdf", g)), width=6*3/inche_cm, height=6/inche_cm, family="ArialMT", colormodel = "cmyk")
