...
if (length(args) == 7) {
  ...
  tree_cutoff <- as.numeric(args[5])
  min_group_gene <- as.integer(args[6])
  f_out <- args[7]
} else {
  q()
}
...
rownames(mergeSCE)
print(marker_df$Gid)
print(rownames(mergeSCE) %in% marker_df$Gid)
...
compute_gene_distance <- function(corrected_mat, cell_indx_li1, cell_indx_li2){
  dim1 <- length(cell_indx_li1)
  dim2 <- length(cell_indx_li2)
  
  mat1 <- corrected_mat[,cell_indx_li1]
  mat2 <- corrected_mat[,cell_indx_li2]
  ...
  dim1_dist <- apply(dist_mat, 1, min)
  dim2_dist <- apply(dist_mat, 2, min)
...
compute_dist_mat <- function(pos, corrected_mat, expr_mat){
  i <- pos[1]
  j <- pos[2]
  cell_indx_li1 <- which(expr_mat[i,])
  cell_indx_li2 <- which(expr_mat[j,])
  tmp_dist <- compute_gene_distance(corrected_mat, cell_indx_li1, cell_indx_li2)
  return(tmp_dist)
}
...
res <- bplapply(index_pair, compute_dist_mat, corrected_mat=corrected_mat, expr_mat=expr_mat, BPPARAM=MulticoreParam(workers=30))
